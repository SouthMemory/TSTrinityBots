...
StatBoostMgr::StatType StatBoostMgr::ScoreItem(Item* item, bool hasAdditionalSpells)
{
    ...
    {
        TC_LOG_INFO("module", "Found {} stats on item.", itemTemplate->StatsCount);
    }
    ...
    for (uint32 i = 0; i < itemTemplate->StatsCount; i++)
    {
        ...
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "StatType: {}, StatValue: {}", stat.ItemStatType, stat.ItemStatValue);
        }
    ...
    if (hasAdditionalSpells)
    {
        ...
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "Found {} spells on item.", itemTemplate->StatsCount);
        }
        ...
        for (_Spell const &spell : itemTemplate->Spells)
        {
            if (spell.SpellId)
            {
                ...
                if (sBoostConfigMgr->VerboseEnable)
                {
                    TC_LOG_INFO("module", "SpellId: {}", spell.SpellId);
                }
    ...
    if (item->GetItemRandomPropertyId() != 0)
    {
        ...
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "Found random property {}.", propId);
        }
        ...
        for (uint32 i = 0; i < 5; i++)
        {
            ...
            if (spellItemEntry->Effect[0] != ITEM_ENCHANTMENT_TYPE_STAT)
            {
                continue;
            }

            auto statType = spellItemEntry->EffectArg[0];
            if (statType == 0)
            {
                continue;
            }
            ...
            if (sBoostConfigMgr->VerboseEnable)
            {
                TC_LOG_INFO("module", "Found random enchant {} with statType {}", spellItemEntry->Name[0], statType);
            }
    ...
    else
    {
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "No random enchants found.");
        }
    }
    ...
    if (!winningScore)
    {
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "No winning score found.");
        }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Finding winning score.");
    }
    for (auto score : roleScores)
    {
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "Score: {}, Type: {}", score->Score, score->stat_type);
        }
    ...
    if (winningScore->Score < 1)
    {
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "No stats were scored.");
        }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed Scoring with scores: Tank({}), Phys({}), Spell({}), Hybrid({})", tankScore.Score, physScore.Score, spellScore.Score, hybridScore.Score);
    }

    return winningScore->stat_type;
}
...
void StatBoostMgr::MakeSoulbound(Item* item, Player* player)
{
    ...
    if (itemTemplate->Bonding == ItemBondingType::BIND_WHEN_EQUIPED)
    {
...
bool StatBoostMgr::BoostItem(Player* player, Item* item, uint32 chance)
{
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed Equipment Check");
    }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed Quality Check. Quality({})", item->GetTemplate()->Quality);
    }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        ...
        TC_LOG_INFO("module", "Passed Roll Check. Roll({})", roll);
    }
    ...
    if (statType == STAT_TYPE_NONE)
    {
        ...
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "No stat type found, got from subclass.");
        }
    }

    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed Analyze Check. StatType({})", statType);
    }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed ItemClass Check");
    }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed ItemSubClass Check");
    }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed ItemType Check");
    }

    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", ">> Trying to get enchant with role mask {}, class {}, subClass {}, itemType {}, and itemlevel {} from pool.", statType, itemClassMask, itemSubClassMask, itemTypeMask, itemLevel);
    }
    ...
    auto enchant = sBoostConfigMgr->EnchantPool.GetByPlayerLevel(statType, itemClassMask, itemSubClassMask, itemTypeMask, player->GetLevel());
    ...
    if (!enchant)
    {
        if (sBoostConfigMgr->VerboseEnable)
        {
            TC_LOG_INFO("module", "Failed Enchant Check.");
        }
    ...
    if (sBoostConfigMgr->VerboseEnable)
    {
        TC_LOG_INFO("module", "Passed Enchant Check. Enchant({})", enchant->Id);
    }
